  ...
  evaluate(p, x) {
    ...
    function _eval(p, x, offset, step, n) {
      ...
      const res = F.add(
        ...
        F.mul(x, _eval(p, newX, offset + step, step << 1, n >> 1))
      );
      return res;
    }
  }
  ...
  square(a) {
    ...
    return [
      this.F.sub(
        this.F.mul(
          this.F.add(a[0], a[1]),
          this.F.add(a[0], this._mulByNonResidue(a[1]))
        ),
        this.F.add(ab, this._mulByNonResidue(ab))
      ),
      this.F.add(ab, ab),
    ];
  }
...
class F3Field {
  ...
  square(a) {
    ...
    return [
      this.F.add(s0, this._mulByNonResidue(s3)), // s0 + non_residue * s3,
  ...
  eq(a, b) {
    return (
      this.F.eq(a[0], b[0]) && this.F.eq(a[1], b[1]) && this.F.eq(a[2], b[2])
    );
  }
  ...
  toString(a) {
    return `[ ${this.F.toString(a[0])} , ${this.F.toString(
      a[1]
    )}, ${this.F.toString(a[2])} ]`;
  }
...
const pTSizes = [
  1, 1, 1, 1, 2, 3, 4, 5, 6, 7, 7, 8, 9, 10, 11, 12, 13, 13, 14, 15, 16, 16, 17,
...
function buildMultiexp(curve, groupName) {
  ...
  async function _multiExpChunk(
    ...
    if (!(buffScalars instanceof Uint8Array)) {
      ...
      throw new Error(
        `${logText} _multiExpChunk buffScalars is not Uint8Array`
      );
    }
    inType = inType || "affine";
  ...
  async function _multiExp(buffBases, buffScalars, inType, logger, logText) {
    ...
    for (let i = 0; i < nPoints; i += chunkSize) {
      ...
      const buffScalarsChunk = buffScalars.slice(
        i * sScalar,
        (i + n) * sScalar
      );
      opPromises.push(
  ...
  G.multiExp = async function multiExpAffine(
    buffBases,
    buffScalars,
    logger,
    logText
  ...
  G.multiExpAffine = async function multiExpAffine(
    buffBases,
    buffScalars,
    logger,
    logText
...
function buildFFT(curve, groupName) {
  ...
  async function _fft(buff, inverse, inType, outType, logger, loggerTxt) {
    ...
    for (let i = 0; i < nChunks; i++) {
      if (logger)
        logger.debug(`${loggerTxt}: fft ${bits} mix start: ${i}/${nChunks}`);
      const task = [];
      ...
      promises.push(
        tm.queueAction(task).then((r) => {
          if (logger)
            logger.debug(`${loggerTxt}: fft ${bits} mix end: ${i}/${nChunks}`);
          return r;
        })
      );
    }
    ...
    for (let i = l2Chunk + 1; i <= bits; i++) {
      if (logger)
        logger.debug(`${loggerTxt}: fft  ${bits}  join: ${i}/${bits}`);
        ...
        for (let k = 0; k < nChunksPerGroup / 2; k++) {
          ...
          opPromises.push(
            tm.queueAction(task).then((r) => {
              if (logger)
                logger.debug(
                  `${loggerTxt}: fft ${bits} join  ${i}/${bits}  ${
                    j + 1
                  }/${nGroups} ${k}/${nChunksPerGroup / 2}`
                );
              return r;
            })
          );
  ...
  G.lagrangeEvaluations = async function (
    buff,
    inType,
    outType,
    logger,
